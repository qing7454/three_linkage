...
import com.qing.common.SQLConsts;
...
public class StringUtil {
	...
	public static String literalizeParam(String str) {
		...
		for (int i = 0; i < patterns2BeEscaped.length; i++) {
			newStr = newStr.replaceAll(patterns2BeEscaped[i],
					patternsOfReplacement[i]);
		}
	 ...
	 */
	public static String cutString(String aString, int specialCharsLength,
			int row) {
	 ...
	 * @return 分割处理后的字符串List
	 */
	private static List<String> subStr(String str, int specialCharsLength,
			int row) {
	...
	public static boolean isExistSpecialStr(String str, String specialStr) {
		...
		if (!StringUtil.isEmptyOrNull(str)
				&& !StringUtil.isEmptyOrNull(specialStr)) {
	...
	public static String stringArrayToString(String[] arrayString,
			String splitStr, Boolean noComma) {
		if (arrayString == null)
			return null;
