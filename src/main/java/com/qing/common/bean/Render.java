...
public abstract class Render {
		...
		while (localIterator.hasNext()) {
			...
			if (!(localEntry.getKey() instanceof String)) {
				throw new IllegalArgumentException(
						"Invalid key [" + localEntry.getKey() + "] in model Map: only Strings allowed as model keys");
			}
			...
			if (localObject != null) {
				...
				if (this.logger.isDebugEnabled())
					this.logger.debug("Added model object '" + str + "' of type [" + localObject.getClass().getName()
							+ "] to request in view ");
			} else {
	...
	public abstract void render(HttpServletRequest paramHttpServletRequest,
